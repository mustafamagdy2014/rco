...
import { ResearcherVisitAttachment } from './../../../domain/ResearcherVisitAttachment';
import { ResearcherVisitAttachmentService } from './../../../service/researcher-visit-attachment.service';
import { PersonAddress } from './../../../domain/PersonAddress';
import { ResearcherVisitService } from './../../../service/researcher-visit.service';
import { ResearcherVisit } from './../../../domain/ResearcherVisit';
...
@Component({
  selector: 'researcher-visit',
  templateUrl: './researcher-visit.component.html',
  styleUrls: ['./researcher-visit.component.css'],
  providers:[ResearcherVisitService, ResearcherVisitAttachmentService]
})
export class ResearcherVisitComponent implements OnInit {
  ...
  researcherVisit:ResearcherVisit = new ResearcherVisit();
  selectedResearcherVisit:ResearcherVisit = new ResearcherVisit();
  researcherVisitsList:ResearcherVisit[];
  ...
  constructor(private globals:Globals, private appService:AppService, private researcherVisitService:ResearcherVisitService, private researcherVisitAttachmentService:ResearcherVisitAttachmentService, private confirmationService:ConfirmationService,private utilityService:UtilityService ) {
  ...
  ngOnInit() 
  {
    ...
    this.researcherVisitService.getResearcherVisitsByFamilyId(this.family.id).subscribe(result=>{this.researcherVisitsList = <ResearcherVisit[]> result;});
    this.cols = 
    [
      { header: 'التسلسل' },
  ...
  { 
    ...
    (result=>
      {
        //load visits list
        this.researcherVisitsList = <ResearcherVisit[]> result;
      }
    );
  }
  ...
  showVisitAttachments(researcherVisitIndex:number)
  {
    ...
    (
      result=>
      {
        this.selectedResearcherVisit.attachments=<ResearcherVisitAttachment[]>result;
        this.displayVisitAttachmentsDialog=true;
        //construct fileUploadURL to contain selected visitId
        this.uploadURL = Resource.UPLOAD_RESEARCHER_VISIT_ATTACHMENT_URL+"/"+this.selectedResearcherVisit.id;
      }
    )
  }
  ...
  deleteResearcherVisit(data)
  {
    ...
    this.researcherVisitService.deleteResearcherVisit(visitId, familyId).subscribe(result=>{this.researcherVisitsList = <ResearcherVisit[]> result;});
  }
}
